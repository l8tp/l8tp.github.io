...
var RootPath = false; //定义不是首页
if("menu" == parentDirectoryName){XML = 'fen-lei.xml'; RootPath = true} //判断当前是否为首页
...
fetch(XML)
	...
	.then(data => {  //处理解析后的文本数据
		...
		links.forEach(link => {  //遍历每一个link
			...
			if(url.includes("/")){ //判断是否为首页按钮
				if (url.split('/').slice(-2, -1)[0] == parentDirectoryName ) {li.className = 'Active';} //设置当前列表属性
			}else{
				if (RootPath == true ) {li.className = 'Active';} //设置当前列表属性
			}
			//以上用来根据上级文件夹名字判断是否为当前按钮，并突出显示
